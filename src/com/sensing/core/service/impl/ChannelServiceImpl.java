...
public class ChannelServiceImpl implements IChannelService{
	...
	private void callAddStreamMedia(String channelAddr){
		try {
			...
			String url = "http://" + PropUtils.getString("streamMedia.ip") + ":" + PropUtils.getString("streamMedia.port") + "/mediaserver/proxy/add";
			
			String post = HttpDeal.post(url, param, 1);
			
			JSONObject resultJson = JSONObject.parseObject(post);
			if ( resultJson != null && resultJson.getString("error") != null && resultJson.getString("message") != null  ) {
				if ( "200".equals(resultJson.getString("error")) ) {
					log.info("流媒体服务添加通道调用正常，返回结果message:"+resultJson.getString("message"));
				}else{
					log.error("流媒体服务添加通道调用失败，返回错误码error:"+resultJson.getString("error")+";返回结果message:"+resultJson.getString("message"));
				}
			}
		} catch (Exception e) {
	...
	private void callRemoveStreamMedia(String channelAddr){
		try {
			...
			String url = "http://" + PropUtils.getString("streamMedia.ip") + ":" + PropUtils.getString("streamMedia.port") + "/mediaserver/proxy/delete";
			...
			if ( resultJson != null && resultJson.getString("error") != null && resultJson.getString("message") != null  ) {
				if ( "200".equals(resultJson.getString("error")) ) {
					...
					log.error("流媒体服务删除通道调用失败，返回错误码error:"+resultJson.getString("error")+";返回结果message:"+resultJson.getString("message"));
				}
			}
		} catch (Exception e) {
	...
	private void chooseStreamMethod(String orignChannelAddr,String newChannelAddr){
		...
		}else if(StringUtils.isNotEmpty(orignChannelAddr) && StringUtils.isNotEmpty(newChannelAddr) && !orignChannelAddr.equals(newChannelAddr)){
	...
	private void callUpdateStreamMedia(String orignChannelAddr,String newChannelAddr){
		try {
			...
			String url = "http://" + PropUtils.getString("streamMedia.ip") + ":" + PropUtils.getString("streamMedia.port") + "/mediaserver/proxy/update";
			String post = HttpDeal.post(url, param, 1);
			...
			if ( resultJson != null && resultJson.getString("error") != null && resultJson.getString("message") != null  ) {
				if ( "200".equals(resultJson.getString("error")) ) {
					...
					log.error("流媒体服务修改通道调用失败，返回错误码error:"+resultJson.getString("error")+";返回结果message:"+resultJson.getString("message"));
				}
			}
		} catch (Exception e) {
	 ...
	 * 修改通道
	 * author dongsl
	 * date 2017年8月21日下午3:13:57
	 */
	public Channel updateChannel(Channel channel)  throws Exception{
		
		try{
			  Channel channelNowInDb = channelDAO.getChannelAll(channel.getUuid());
			  if(channelNowInDb !=null){
				...
				//通知调用流媒体服务修改通道
				  ...
				  if(Constants.CAP_STAT_OPEN.equals(channel.getCapStat()))
				  {
	 ...
	 */
	private void CallRemoveChannel(String uuid){
	...
	private void CallModifyChannel(Channel channel){
		...
		if (channel.getChannelUid() != null
				&& !(channel.getChannelUid().isEmpty())) {
			capChannel.setChannel_uid(channel.getChannelUid());// 登录视频源用户名
		}
		if (channel.getChannelPsw() != null
				&& !(channel.getChannelPsw().isEmpty())) {
		...
		if (channel.getChannelAddr() != null
				&& !(channel.getChannelAddr().isEmpty())) {
		...
		if (channel.getChannelNo() != null
				&& !(channel.getChannelNo().isEmpty())) {
		...
		try {
			...
			if(StringUtils.isNotEmpty(channel.getNodeType()) && channel.getCapStat() == Constants.CAP_STAT_CLOSE){
				rl.setTodo("关闭通道");
			}else if(StringUtils.isNotEmpty(channel.getNodeType()) && channel.getCapStat() == Constants.CAP_STAT_OPEN){
				rl.setTodo("开启通道");
			}else{
				rl.setTodo("修改通道");
			} 
	...
	private void CallAddChannel(Channel channel){
		...
		/*if (channel.getChannelDescription() != null && !(channel.getChannelDescription().isEmpty())) {
...
//		capChannel.setStrReserve("filter:1,3");
//		capChannel.setStrReserve("filter:1,3");
//		capChannel.setStrReserve("filter:1,3");
	...
	@Override
	public List<Channel> queryChannelByChannelNameAndRegionId(String channelName, Integer regionId, short isDel) throws Exception {
			...
			if(regionId!=null){
				...
				if(region!=null){
			        ...
			        if(maxCode==null){
			        	...
			        	newSeachCode = maxCode.substring(0,maxCode.length()-4) + f.format(Integer.parseInt(last4Number)+1);
			        }
	 ...
	 */
	@Override
	public ChannelQueryResult QueryChannelsByRegionID(Integer regionID,Integer nStartNum, Integer nCount) throws Exception {
		...
		try {
			...
			log.info("调用初始化通道方法，参数：regionId--" + regionID + ",startNum--" + nStartNum + ",nCount--" + nCount + ",orgIdList:" + idList.toString());
