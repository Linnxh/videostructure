...
public class RegionsController extends BaseController {
    ...
    public ResponseBean save(@RequestBody JSONObject m, HttpServletRequest request) {
        ...
        try {
            ...
            if (StringUtils.isNotEmpty(valRes)) {    //如果验证通过，则进行下一步，否则返回错误信息
                ...
                if (model.getRegionLevel() > 4) {
                    result.setError(100);
                    result.setMessage("分组层级不能超过5层");
                    return result;
                }
    ...
    /**
     * 保存区域信息之前验证	type=1验证save方法，type=2验证update方法
     * author dongsl
    ...
    private String validateParam(Regions region, int type) throws Exception {
        ...
        if (region.getParentId() != null) {
            list = regionsService.queryRegionByParentId(String.valueOf(region.getParentId()), region.getRegionName(), region.getId());
        } else {
